...
import { Save, AlertCircle, CheckCircle2, Search, Lock, Cpu, Filter, BookOpen, Circle, AlertTriangle, CalendarOff } from 'lucide-react';
...
const GradeRegistrationView: React.FC<GradeRegistrationViewProps> = ({ 
  ...
  const shouldShowColumn = (periodKey: string) => {
      // Admins and Administrative staff see EVERYTHING always (for corrections)
      if (['admin', 'subdirector', 'administrative'].includes(currentUser.role)) {
          return true;
      }
      // Teachers only see ACTIVE periods
      return data.allowedPeriods.includes(periodKey);
  };

  // CHECK IF SYSTEM IS LOCKED FOR TEACHER (No active periods)
  const isSystemLockedForTeacher = currentUser.role === 'teacher' && data.allowedPeriods.length === 0;
  ...
  return (
    <div className="p-6 md:p-8 space-y-6 h-full flex flex-col">
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
            <div>
             ...
             {showTable && !isSystemLockedForTeacher && (
                <button 
                    onClick={handleSave}
                    disabled={isSaving}
        ...
        {/* SELECTOR BAR (Siempre visible para navegar) */}
      ...
      {/* --- TABLE AREA OR LOCKED MESSAGE --- */}
      <div className="flex-1 overflow-hidden bg-white rounded-2xl shadow-sm border border-slate-200 flex flex-col relative">
        
        {/* MESSAGE: SYSTEM LOCKED (TEACHER ONLY) */}
        {isSystemLockedForTeacher ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 bg-slate-50/50 p-8 text-center animate-fade-in">
                <div className="p-6 bg-white rounded-full shadow-sm mb-4 border border-slate-200">
                    <CalendarOff size={64} className="text-slate-300" />
                </div>
                <h3 className="text-2xl font-bold text-slate-600 mb-2">Periodo de Captura Cerrado</h3>
                <p className="text-slate-500 max-w-md">
                    Actualmente no hay periodos de evaluación habilitados. Por favor espera a que la Dirección habilite el sistema para el próximo corte.
                </p>
            </div>
        ) : !showTable ? (
