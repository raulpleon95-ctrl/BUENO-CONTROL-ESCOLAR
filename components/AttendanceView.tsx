...
import { CalendarCheck, Save, Printer, ArrowLeft, CheckCircle2, XCircle, Clock, FileWarning, Check } from 'lucide-react';
...
const AttendanceView: React.FC<AttendanceViewProps> = ({ data, currentUser, onUpdateData, onBack }) => {
  // Modes: Capture (Daily) or Report (Monthly)
  const [mode, setMode] = useState<'capture' | 'report'>('capture');
  ...
  useEffect(() => {
      if (mode === 'capture' && selectedGrade && selectedGroup && selectedDate) {
          ...
          // If no record exists for a student, default is undefined (will show as unmarked)
          // Optionally, we could default to 'present' on load, but better to be explicit.
  ...
  const getStatusIcon = (status?: AttendanceStatus) => {
      switch(status) {
          ...
          case 'excused': return <FileWarning size={16}/>;
          default: return null;
      }
  };
      ...
      <div className="p-6 md:p-8 space-y-6 print:hidden bg-slate-50 border-b border-slate-200">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                ...
                <p className="text-slate-500">Registro diario y reportes mensuales de inasistencias.</p>
            </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 flex flex-wrap gap-4 items-end">
             ...
             {mode === 'capture' ? (
                 <div className="flex flex-col">
                    <label className="text-xs font-bold text-slate-500 uppercase mb-1">Fecha de Asistencia</label>
                    <input 
                        type="date" 
                        value={selectedDate} 
          ...
          {mode === 'report' && (
              <div className="bg-white shadow-lg mx-auto p-[10mm] print:shadow-none print:w-full print:max-w-none text-black">
                   ...
                   <div className="mt-8 flex justify-between text-xs font-bold uppercase text-center px-12">
                        ...
                        <div>
                            <div className="border-t border-black pt-2 w-48 mx-auto">{data.director}</div>
                            <p>Vo. Bo. DIRECTOR</p>
                        </div>
                   </div>
              </div>
